...
import { ShieldCheck, UserPlus, Loader2, CheckCircle, AlertTriangle } from 'lucide-react';
import Link from 'next/link';

export default function AccesosPage() {
  const { supabase } = useSupabase();
  const [simuladores, setSimuladores] = useState<any[]>([]);
  const [selectedSimulador, setSelectedSimulador] = useState('');
  const [email, setEmail] = useState('');
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState<{ type: 'success' | 'error', text: string } | null>(null);

  // Cargar la lista de simuladores al abrir la página
  useEffect(() => {
  ...
  const handleDarAcceso = async (e: React.FormEvent) => {
    ...
    try {
      // Llamamos a la función mágica de la base de datos
      const { error } = await supabase.rpc('dar_acceso_simulador', {
        p_email: email.trim().toLowerCase(),
        p_simulador_id: selectedSimulador
      });

      if (error) throw error;

      setMessage({ type: 'success', text: `¡Acceso otorgado correctamente a ${email}!` });
      setEmail(''); // Limpiamos el correo para que puedas ingresar el siguiente rápido
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Otorga acceso a simuladores privados a tus estudiantes usando solo su correo.</p>
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
        <form onSubmit={handleDarAcceso} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">1. Selecciona el Simulador</label>
            <select
              value={selectedSimulador}
              onChange={(e) => setSelectedSimulador(e.target.value)}
              className="w-full p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary bg-gray-50 text-gray-800"
              required
            >
              <option value="" disabled>-- Elige un simulador de la lista --</option>
              {simuladores.map((sim) => (
                <option key={sim.id} value={sim.id}>
                  [{sim.institucion}] - {sim.nombre}
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full p-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary focus:border-primary bg-gray-50"
              required
            />
